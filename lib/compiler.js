...
module.exports = function compile(options) {
  ...
  if (options.appId)
	...
	debug('raw instructions before env vars substitution:', instructions);

	// It is not expected to have `undefined`/`date` variables inside configs, so we can safely use JSON.[stringify/parse]
	// The below code replaces all the occurrances of pattern like ${ENV_VAR} with process.env[ENV_VAR] if available.
	instructions = JSON.stringify(instructions).replace(/\$\{(.*?)\}/g, function(match, p1, offset, str) {
      if(process.env[p1]) {
        return process.env[p1];
      }
	  console.warn('WARNING: ENV VAR "' + p1 + '" is missing.');
      return match;
	});

	instructions = JSON.parse(instructions);

	debug('processed instruction after substituting env vars:', instructions);
